...
select t.tipo as TIPO,t.numero as NUMERO,isnull(c.nombres+' '+c.ape_paterno+' '+c.ape_materno,'SIN_DETALLE') as CLIENTE,
isnull(c.email,'SIN DETALLE') as EMAIL,convert(varchar(8),getdate(),112) as FEC_CONSULTA
from Telefono t
left join Cliente c on t.codcliente=c.codcliente
where t.estado=1 and c.tipo='P'--teléfonos en estado 'activo' de los clientes persona
